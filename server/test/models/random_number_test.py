...
class RandomNumberDrawTestCase(TestCase):
    ...
    def parametrized_constructor_test(self):
        """Test parametrized constructor for RandomNumberDraw"""
        tested_item = RandomNumberDraw(range_max = 5,allow_repeat=True, number_of_results = 1)
        self.assertEqual(tested_item.range_min,0)
    ...
    def is_feasible_default_test(self):
        """Default RandomNumberDraw is not feasible"""
        self.assertFalse(RandomNumberDraw().is_feasible())

    def is_feasible_simple_test(self):
        """Simple RandomNumberDraw is feasible"""
        tested_item = RandomNumberDraw(range_max=5)
        self.assertTrue(tested_item.is_feasible())

    def is_feasible_limit_ok_test(self):
        """Border case for RandomNumberDraw: is feasible"""
        tested_item = RandomNumberDraw(range_max=5,range_min=2,number_of_results=3,allow_repeat=False)
        self.assertTrue(tested_item.is_feasible())

    def is_feasible_limit_ko_test(self):
        """Border case for RandomNumberDraw: is not feasible"""
        tested_item = RandomNumberDraw(range_max=5,range_min=2,number_of_results=4,allow_repeat=False)
        self.assertFalse(tested_item.is_feasible())

    def is_feasible_limit_with_repeat_ok_test(self):
        """Border case for RandomNumberDraw: is feasible with repeat"""
    ...
    def is_feasible_range_ko_test(self):
        """Border case for RandomNumberDraw: Range not feasible"""
        tested_item = RandomNumberDraw(range_max=2,range_min=4)
        self.assertFalse(tested_item.is_feasible())

    def is_feasible_range_with_repeat_ko_test(self):
        """Border case for RandomNumberDraw: Range not feasible with repeat"""
