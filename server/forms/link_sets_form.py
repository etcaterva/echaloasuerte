...
class LinkSetsDrawForm(FormBase):
    ...
    def __init__(self, *args, **kwargs):
        if 'initial' in kwargs and 'sets' in kwargs['initial']:
            for i in range(0, len(kwargs['initial']['sets'])):
                kwargs['initial']['set_{0}'.format(i + 1)] = ','.join(
                    kwargs['initial']['sets'][i])
        ...
        self.fields['set_1'].widget.attrs.update(
            {'class': 'protected eas-tokenfield'})
        self.fields['set_2'].widget.attrs.update(
            {'class': 'protected eas-tokenfield'})
    ...
    def clean(self):
        ...
        if not self._errors:
            ...
            proc_set1 = raw_set1.split(
                ",") if ',' in raw_set1 else raw_set1.split()
            proc_set2 = raw_set2.split(
                ",") if ',' in raw_set2 else raw_set2.split()
