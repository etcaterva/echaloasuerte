...
/**
 * Spins a a spinable div by rotating it until it reaches a
 *  specified angle degree
 * {img} image to spin
 * {Number} degree final degree to end up when spinning
 * Sample html to apply it to
    <img class="spinable" src="http://placehold.it/200x200" alt="" />
 */
function spin(spinable, degree){
    spinable.css({'rotate': '0deg'});
    spinable.transition({
        rotate: Math.floor((Math.random()*10)+5)*360+degree+'deg',
        duration: 2000,
        easing: 'cubic-bezier(0.100, 0.300, 0.700, 1.020)'
    });
}
